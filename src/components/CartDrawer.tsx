    ...
    <>
      ...
      <div className="fixed right-0 top-0 bottom-0 w-full max-w-md bg-card border-l border-border z-50 flex flex-col animate-fade-in-up">
        <div className="flex items-center justify-between p-4 border-b border-border">
          <h2 className="font-heading text-xl font-bold">{t('cart.title')} ({totalItems})</h2>
            ...
            items.map(item => (
              <div key={`${item.productId}-${item.weight}`} className="flex gap-3 p-3 bg-muted/30 rounded-lg">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {item.isGift ? (
                    <span className="text-xs font-bold text-accent">🎁 {t('cart.free')}</span>
                  ) : (
                    <div className="flex items-center justify-between mt-2">
                      ...
                      <span className="text-sm font-bold text-primary">{(item.price * item.quantity).toFixed(3)} {t('products.dt')}</span>
                    </div>
                  )}
                </div>
              </div>
        ...
        {items.length > 0 && (
          <div className="p-4 border-t border-border space-y-3">
            ...
            {promoCode && (
              <p className="text-xs text-accent">✅ Code {promoCode} : −{promoDiscount * 100}%</p>
            )}
            <div className="flex justify-between text-sm">
              <span className="text-muted-foreground">{t('cart.subtotal')}</span>
              <span>{subtotal.toFixed(3)} {t('products.dt')}</span>
            </div>
            {promoDiscount > 0 && (
              <div className="flex justify-between text-sm text-accent">
                <span>{t('cart.discount')}</span>
                <span>−{(subtotal * promoDiscount).toFixed(3)} {t('products.dt')}</span>
              </div>
            )}
            <div className="flex justify-between text-lg font-bold">
              <span>{t('cart.total')}</span>
              <span className="text-primary">{total.toFixed(3)} {t('products.dt')}</span>
            </div>
            <button
